...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      // Check if trying to log in as admin from the frontend
...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    if (!validator.isStrongPassword(password, { minLength: 8, minLowercase: 1, minUppercase: 1, minNumbers: 1, minSymbols: 1 })) {
      return res.status(400).json({ success: false, message: "Password must be at least 8 characters long and include uppercase, lowercase, numbers, and symbols" });
    }
...
const googleLogin = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      // Account Merging: update googleId/image if missing
      if (!user.googleId) user.googleId = googleId;
      if (!user.image)    user.image    = picture;
      await user.save();
    } else {
      // New user: create account without a password
      user = new userModel({ name, email, googleId, image: picture });
      await user.save();
    }
